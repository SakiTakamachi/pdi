...
static zend_result pdi_create_instance(pdi_object_t *pdi, pdi_concrete_t *concrete, const zend_string *abstract, HashTable *args, zval *instance);

static zend_result pdi_get_singleton(pdi_object_t *pdi, const zend_string *abstract, zval *instance)
{
    instance = zend_hash_str_find_ptr(pdi->singletons, ZSTR_VAL(abstract), ZSTR_LEN(abstract));
    return SUCCESS;
}

static zend_result pdi_get_instance(pdi_object_t *pdi, const zend_string *abstract, HashTable *args, zval *instance)
{
